...
/*
	b = number of rows
	u = highest value of piece, limited to 1-10 by images right now
 */
    this.base = b;
    this.pieces = [];
    this.upper = u;
...
*/		
    ...
    _('lvl').innerHTML = '级别: ' + (lvl - 1).toString();
    _('ans').innerHTML = '';
    _('panel').innerHTML = '';
    _('results').innerHTML = '';
    ...
    _('cur').innerHTML = '当前值: 0';
    var g = new game(lvl, 9);
    var ans = g.solution();
    _('ans').innerHTML = '目标值:  ' + ans;
    ...
    function lose() {
		...
		*/			
        ...
        _('results').innerHTML = '哈哈，你输了！!<br /><sub>提示: 目标值是所有球的值的和，选择的球的连线不能断哦。</sub>';
        _('results').innerHTML += '<br /><button onClick="stopWin();start(lvl)">爷要再试一次！</button>'
    };
    ...
    function tally() {
		...
		*/		
        ...
        _('cur').innerHTML = '当前值: ' + g.playerTotal;
        if (g.playerTotal == ans && checkWin()) {
            clearBoard();
            playAudio('win');
            g.celebrate(true);
            _('results').style.display = 'block';
            _('results').innerHTML = '可以啊，赢了！!';
